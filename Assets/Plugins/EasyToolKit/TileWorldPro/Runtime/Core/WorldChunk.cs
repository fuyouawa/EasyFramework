...
namespace EasyToolKit.TileWorldPro
{
    public class WorldChunk
    {
        ...
        public IEnumerable<TileBlockInfo> EnumerateTiles()
        {
            foreach (var terrainSection in _terrainSections)
            {
                foreach (var tile in terrainSection.Tiles)
                {
                    yield return new TileBlockInfo
                    {
                        TilePosition = tile.ConvertToTilePosition(),
                        TerrainGuid = terrainSection.TerrainGuid,
                    };
                }
            }
        }

        public IEnumerable<TileBlockInfo> EnumerateTerrainTiles(Guid terrainGuid)
        {
            var terrainSection = _terrainSections.FirstOrDefault(section => section.TerrainGuid == terrainGuid);
            if (terrainSection == null)
            {
