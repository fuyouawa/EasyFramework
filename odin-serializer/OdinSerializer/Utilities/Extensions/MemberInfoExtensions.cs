...
namespace EasyToolKit.Core.Internal.OdinSerializer.Utilities
{
    using System;
    ...
    public static class MemberInfoExtensions
    {
        ...
        public static bool IsDefinedBase<T>(this ICustomAttributeProvider member, bool inherit = false) where T : Attribute
        {
            return member.IsDefinedBase(typeof(T), inherit);
        }

        public static bool IsDefinedBase(this ICustomAttributeProvider member, Type attributeType, bool inherit = false)
        {
            try
            {
                if (member.IsDefined(attributeType, inherit))
                {
                    return true;
                }

                return member.GetCustomAttributes(inherit).Any(attr => attributeType.IsAssignableFrom(attr.GetType()));
            }
            catch
            {
                return false;
